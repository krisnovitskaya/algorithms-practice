...
Может быть 2 крайних случая:
1) H = logN, когда на входе оказыается сбалансированное дерево => сложность будет O(logN)
2) H = N, когда на входе вырожденное в связанный список дерево => сложность
в худшем случае будет (удаляемый элемент последний) = O(N)
То есть в среднем (O(logN) + O(N))/2 -> O(logN+N)
